...
def train_model(model, device, dataloaders, criterion, optimizerDict, loss_fn_pos_weight, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # deep copy the model with best mcc, and save the corresponding metrics
...
def dataPreprocess_cv(batch_size, data_dir, seed):
    ...
    transform = transforms.Compose([transforms.Resize((64,64), interpolation=2),
                                    transforms.ToTensor()
                                   ])
...
if __name__ == "__main__":
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    pkl_file = open(log_dir+'mlp_img_'+op+'_cv'+'.pickle','wb') # save as pickle with the same file name
